...
FROM {{ ref('int_laptop_model_latest') }} AS l
LEFT JOIN {{ ref('dim_brand') }} AS b ON l.brand_id = b.brand_id
LEFT JOIN {{ ref('dim_cpu') }} AS c ON l.cpu_model_id = c.cpu_model_id
LEFT JOIN {{ ref('dim_gpu') }} AS g ON l.gpu_model_id = g.gpu_model_id
LEFT JOIN {{ ref('dim_laptop_benchmark') }} AS bm ON l.laptop_id = bm.laptop_id